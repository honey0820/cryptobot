...
print("Reading trading view data...")
...
print("Calculating trading view data...")
...
print("Displaying trading view data...")
# Create the plot
...
print("Sending email with trading signal...")

maildata= open(".password","r",encoding="utf-8").read().split("\n")
# print(maildata)
mygmail = maildata[0]
mygmailpassword = maildata[1]
togmail = maildata[2]
contents = [f"Buy signals: {buy_signals}, Sell signals: {sell_signals}"]
try:
    yag = yagmail.SMTP(mygmail, mygmailpassword)
    yag.send(togmail, 'Signal From PyChatGPTBot', contents)
